    ...
    while (mp4->offset < end) {
            ...
            || mp4->offset + (off_t) atom_size > end)
        {
            ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                          ...
                          mp4->file.name.data, atom_size);
            if(ngx_strncmp(atom_name, "mdat", sizeof("mdat")-1) != 0) {
                return NGX_ERROR;
            }              
        }
...
ngx_http_mp4_read(ngx_http_mp4_file_t *mp4, size_t size)
{
    ...
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                      "ngx_http_mp4_read(%uD) failed, pos=%p,end=%p", size, mp4->buffer_pos, mp4->buffer_end);
...
static ngx_int_t
ngx_http_mp4_update_mvhd_atom(ngx_http_mp4_file_t *mp4, uint64_t start) {
    ngx_buf_t              *data;
    u_char                 *atom_header;
    uint32_t                timescale;
    uint64_t                start_time, duration, length_time;
    int64_t                 delta;
    ngx_mp4_mvhd_atom_t    *mvhd_atom;
    ngx_mp4_mvhd64_atom_t  *mvhd64_atom;
    if(0)
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                      "ngx_http_mp4_update_mvhd_atom,start=%uD", start);
    
    data = mp4->mvhd_atom.buf;
    if(data == NULL) {
        return NGX_ERROR;
    }
    
    atom_header = data->pos;
    ...
    delta = (int64_t) (start - mp4->start) * timescale / 1000;
    
    start_time = (uint64_t) start * timescale / 1000;
    duration -= start_time;
    if (mp4->length) {
...
ngx_http_mp4_read_hdlr_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    atom_header = ngx_mp4_atom_header(mp4);
    atom_size = sizeof(ngx_mp4_atom_header_t) + (size_t) atom_data_size;
    handler_type = atom_header + sizeof(ngx_mp4_atom_header_t) + 2*sizeof(int32_t);
    ngx_mp4_set_32value(atom_header, atom_size);
    ngx_mp4_set_atom_name(atom_header, 'h', 'd', 'l', 'r');

    trak = ngx_mp4_last_trak(mp4);

    if(ngx_strncmp(handler_type, "vide", sizeof("vide")-1) == 0) {
        trak->trak_media_type = NGX_HTTP_MP4_VIDEO_TRACK;
    }else if(ngx_strncmp(handler_type, "soun", sizeof("soun")-1) == 0) {
        trak->trak_media_type = NGX_HTTP_MP4_AUDIO_TRACK;
    }else {
        trak->trak_media_type = NGX_HTTP_MP4_UNKNOWN_TRACK;
    }
...
ngx_http_mp4_read_stts_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_http_mp4_trak_t  *trak;
    ngx_uint_t              count, sample_count, duration;
    ...
    if(0)
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                       "track:%uD, read stts atom,sample count:%uD", 
                       trak->trak_id, sample_count);
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    ngx_mp4_stts_entry_t  *entry, *end;
    uint32_t              *sync_array;
    uint32_t               sample_number, sample_index;
    ngx_uint_t             begin, stop, mid, sync_sample_count;
    ngx_int_t  k;

    ngx_mp4_sample_element_t       *samples, element;
    ngx_uint_t             dts, sync_found;

    sync_sample_count = trak->stss_samples.nelts;
    //sync_sample_count = 0;
    if (start) {
        if(trak->trak_media_type == NGX_HTTP_MP4_AUDIO_TRACK) {
            start_sec = mp4->video_sync_start;
        }else {
            start_sec = mp4->start;
            //mp4->video_sync_start = mp4->start;
        }
    ...
    if (start && sync_sample_count > 0)
    {
        sync_found = 0;
        begin = 0;
        stop = sync_sample_count - 1;
        sync_array = (uint32_t *)trak->stss_samples.elts;
        samples = (ngx_mp4_sample_element_t *)trak->stts_samples.elts;
        while (begin <= stop)
        {
            mid = (stop + begin) / 2;
	     if(mid<=0 || mid>=(sync_sample_count-1)){
	          break;
	     }
            sample_number = sync_array[mid];
            element = samples[sample_number-1];
            if (start_time < element.dts)
            {
                stop = mid - 1;
            }
            else if (start_time > element.dts)
            {
                begin = mid + 1;
            }
            else
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "binary search(%d-%d)", begin, stop);
        }

        if (sync_found)
        {
            sample_index = sync_array[mid];
	     if(0){
            ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                          "******************************found sync sampe index:%uD in [%uD]*************************",
                          sample_index, mid);
	     	}
        }
        else
        {
            if (mp4->seek_directed == NGX_HTTP_MP4_SEEK_LEFT_DIRECTED)
            {
                sample_number = sync_array[mid];
                element = samples[sample_number-1];
                if(start_time < element.dts) {
            ...
            else
            {
                ...
                if(start_time < element.dts) {
                     ...
                     //关键帧下标等于mid+1
                     k = mid+1;
                     if(k >= sync_sample_count) {
                         k = sync_sample_count - 1;
                     }
                     sample_index = sync_array[k];
                }
                if(0){
        ...
        element = samples[sample_index];
        mp4->video_sync_start = element.dts * 1000 / trak->timescale;
        ...
        while (entry < end)
        {
            ...
            {
                rest = sample_index + count - start_sample - 1;
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    ngx_uint_t            first_chunk, last_chunk;
    ngx_uint_t            samples_per_chunk, sample_count;
    ...
    sample_count = 0;
    last_chunk = trak->chunks + 1;
    end = (ngx_mp4_stsc_entry_t *) data->pos;
    entry = (ngx_mp4_stsc_entry_t *) data->last;
    entry--;
    
    while (entry >= end ) {
        first_chunk = ngx_mp4_get_32value(entry->chunk);
        samples_per_chunk = ngx_mp4_get_32value(entry->samples);

        //ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
        //           "track:%uD, first_chunk:%uD, samples_per_chunk:%uD",
        //           mp4->trak.nelts - 1, chunk, samples_per_chunk);

        sample_count += (last_chunk - first_chunk) * samples_per_chunk;
        last_chunk = first_chunk;
        entry--;
    }
    if(0)
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                   "track:%uD, read stsc atom, chunks:%uD, sample count:%uD",
                   trak->trak_id, trak->chunks, sample_count);
...
static ngx_int_t
ngx_http_mp4_build_sync_samples_from_stss_atom(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak)
{
    ngx_uint_t                 rc;
    size_t                     atom_size;
    ...
    entries = trak->sync_samples_entries;
    rc = ngx_array_init(&trak->stss_samples, mp4->request->pool, entries, sizeof(uint32_t));
    if(rc != NGX_OK) {
        return rc;
    }
    ...
    while (entry < end) {
        ...
        uint32_t *sample_number = (uint32_t *)ngx_array_push(&trak->stss_samples);
...
static ngx_int_t
ngx_http_mp4_build_samples_from_stts_atom(ngx_http_mp4_file_t *mp4,
    ngx_http_mp4_trak_t *trak)
{
    ngx_int_t             rc, i, j;
    size_t                atom_size;
    ...
    if(0)
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                       "track:%uD, build stts atom,sample count:%uD", 
                       trak->trak_id, sample_count);
    

    rc = ngx_array_init(&trak->stts_samples, mp4->request->pool, sample_count, sizeof(ngx_mp4_sample_element_t));
    if(rc != NGX_OK) {
        return rc;
    }
    entry = (ngx_mp4_stts_entry_t *) data->pos;
    ...
    while (entry < end) {
        ...
        //               count, duration);
        for(j = 0; j < count; j++) {
            ngx_mp4_sample_element_t *sample = (ngx_mp4_sample_element_t *)ngx_array_push(&trak->stts_samples);
            sample->dts =  last_dts + duration;
            last_dts += duration;
        }
...
ngx_int_t ngx_http_mp4_process_metadata(ngx_http_request_t *r,  ngx_http_mp4_file_t  *mp4, ngx_http_mp4_range_t *range)
{
    ...
    ngx_chain_t               **prev;
    ngx_http_mp4_trak_t       *trak;
    ngx_uint_t                 first_video_track;
    ngx_uint_t                 first_audio_track;
    ...
    //根据stts,stss建立sample映射表和sync_sample映射�?
    trak = mp4->trak.elts;
    for (i = 0; i < mp4->trak.nelts; i++) {
        if (ngx_http_mp4_build_samples_from_stts_atom(mp4, &trak[i]) != NGX_OK) {
            return NGX_ERROR;
        }

        if (ngx_http_mp4_build_sync_samples_from_stss_atom(mp4, &trak[i]) != NGX_OK) {
            return NGX_ERROR;
        }
    }
    ...
    first_video_track = 0;
    for (i = 0; i < mp4->trak.nelts; i++) {
        if(trak[i].trak_media_type == NGX_HTTP_MP4_VIDEO_TRACK) {
            first_video_track = 1;
	     if(0)
            ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                      "trak[%d] first_video_track", i);
            break;

        }else if(trak[i].trak_media_type == NGX_HTTP_MP4_AUDIO_TRACK) {
            first_audio_track = 1;
	     if(0)
            ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                      "trak[%d] first_audio_track", i);
            break;
        }
    }
    ...
    *prev = &mp4->moov_atom;
    ...
    mp4->content_length += mp4->moov_size;
/*
    if(ngx_http_mp4_update_mvhd_atom(mp4, mp4->video_sync_start) != NGX_OK) {
        return NGX_ERROR;
    }
*/
    *prev = &mp4->mdat_atom;
